...
async function submitOrder(url, hold = false) {
    ...
    if (!hold && payload.transaction_type !== 'replacement' && payload.payments.reduce((sum, row) => sum + row.amount, 0) < totals().total) return alert('Nominal pembayaran kurang. Isi uang tunai yang diterima untuk melunasi tagihan.');
