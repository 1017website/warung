...
<div class="card card-pad" style="margin-top:16px">
    <div class="card-title"><div><h2>{{ $members->total() }} data member</h2><p>Cari berdasarkan nama, kode kartu, atau QR.</p></div><form class="search" method="GET" style="max-width:340px"><input name="q" value="{{ $search }}" placeholder="Cari nama, kode, atau QR…"></form></div>
    <div class="table-wrap"><table><thead><tr><th>Member</th><th>Kontak / domisili</th><th>Tanggal lahir</th><th>Diskon</th><th>Status</th><th>Saldo deposit</th><th>Aksi</th></tr></thead><tbody>
    @forelse($members as $member)
        <tr>
            ...
            <td><div class="actions">
                ...
                <form method="POST" action="{{ route('members.status',$member) }}">@csrf @method('PATCH')<input type="hidden" name="is_active" value="{{ $member->is_active ? 0 : 1 }}"><button class="btn {{ $member->is_active ? 'btn-danger' : 'btn-soft' }} btn-sm">{{ $member->is_active ? 'Nonaktifkan' : 'Aktifkan' }}</button></form>
            </div></td>
        </tr>
    @empty
...
<div class="modal" id="edit-member-modal"><div class="modal-card"><div class="modal-head"><h2>Edit data member</h2><button class="modal-close" onclick="closeModal('edit-member-modal')">×</button></div><form method="POST" id="edit-member-form" class="form-grid">@csrf @method('PUT')<div class="field full"><label>Nama</label><input name="name" required></div><div class="field"><label>No. HP</label><input name="phone"></div><div class="field"><label>Email</label><input name="email" type="email"></div><div class="field"><label>Domisili</label><input name="domicile"></div><div class="field"><label>Tanggal lahir</label><input name="birth_date" type="date"></div><div class="field full"><label>Diskon (%)</label><input name="discount_percent" type="number" min="0" max="100" step="0.1" required></div><div class="field full"><button class="btn btn-primary">Simpan perubahan</button></div></form></div></div>

<div class="modal" id="adjustment-modal"><div class="modal-card" style="max-width:460px"><div class="modal-head"><div><h2>Koreksi deposit</h2><div id="adjustment-name" class="hint"></div></div><button class="modal-close" onclick="closeModal('adjustment-modal')">×</button></div><form method="POST" id="adjustment-form" class="form-grid">@csrf<div class="field"><label>Arah koreksi</label><select name="type"><option value="credit">Tambah saldo</option><option value="debit">Kurangi saldo</option></select></div><div class="field"><label>Nominal</label><input name="amount" type="text" data-money-input data-min="1" required></div><div class="field full"><label>Alasan audit</label><input name="reason" minlength="5" required></div>@unless(auth()->user()->isSupervisor())<div class="field full"><label>PIN Manager/SPV cabang aktif</label><input name="approval_pin" type="password" inputmode="numeric" required></div>@endunless<div class="field full"><button class="btn btn-primary">Simpan koreksi</button></div></form></div></div>
...
document.querySelectorAll('.adjustment-trigger').forEach(button=>button.addEventListener('click',()=>{document.getElementById('adjustment-form').action=button.dataset.url;document.getElementById('adjustment-name').textContent=button.dataset.name;openModal('adjustment-modal')}));
document.querySelectorAll('.edit-member-trigger').forEach(button=>button.addEventListener('click',()=>{const form=document.getElementById('edit-member-form'),member=JSON.parse(button.dataset.member);form.action=button.dataset.url;Object.entries(member).forEach(([key,value])=>{const input=form.elements[key];if(input)input.value=value||''});openModal('edit-member-modal')}));
