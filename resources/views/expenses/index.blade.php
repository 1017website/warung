...
<div class="card card-pad"><div class="card-title"><div><h2>Riwayat pengeluaran</h2><p>{{ $expenses->total() }} catatan</p></div></div><div class="table-wrap"><table><thead><tr><th>Tanggal</th><th>Keterangan</th><th>Kategori</th><th>Dibayar via</th><th>Nominal</th><th></th></tr></thead><tbody>
@forelse($expenses as $expense)<tr><td>{{ $expense->expense_date->format('d M Y') }}</td><td class="cell-main">{{ $expense->description }}</td><td><span class="badge gray">{{ $expense->category }}</span></td><td>{{ strtoupper($expense->payment_method ?? 'cash') }}</td><td class="money">Rp {{ number_format($expense->amount,0,',','.') }}</td><td><form method="POST" action="{{ route('expenses.destroy',$expense) }}" onsubmit="return confirm('Arsipkan catatan ini?')">@csrf @method('DELETE')<button class="btn btn-danger btn-sm">Arsipkan</button></form></td></tr>@empty<tr><td colspan="6"><div class="cart-empty">Belum ada pengeluaran.</div></td></tr>@endforelse
...
<div class="field full"><label>Dibayar melalui</label><select name="payment_method" required><option value="cash">Tunai</option><option value="transfer">Transfer</option><option value="qris">QRIS</option><option value="debit">Kartu debit</option></select></div>
