...
(async()=>{
 ...
 for(const [role,label] of Object.entries(roles)){
  ...
  await page.pdf({path:path.join(out,`User-Guide-${label.replaceAll(' ','-')}.pdf`),format:'A4',printBackground:true,margin:{top:'15mm',bottom:'17mm',left:'18mm',right:'18mm'},displayHeaderFooter:true,headerTemplate:'<div></div>',footerTemplate:`<div style="font:9px Arial;width:100%;padding:0 18mm;color:#65758a;display:flex;justify-content:space-between"><span>POS Warung · ${label} · 6 September 2026</span><span><span class="pageNumber"></span> / <span class="totalPages"></span></span></div>`,tagged:true,outline:true});
